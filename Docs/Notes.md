...
- 2026-08-25에 중앙 판정 자체를 걷어냈다 — 아래 「모니터 간 이동에서 중앙 판정을 걷어냈다」
...
- 「중앙이 아니면 현재 화면의 중앙으로, 중앙이면 다음 화면으로」는 옆 모니터까지 가는 데 두 번이 든다.
  누른 횟수와 넘어간 모니터 수가 어긋나 손에 익지 않는다
- 커서 위치를 따지지 않고 바로 다음 화면으로 보낸다. 좌표 오차에 걸려 순환이 시작되지 않는 문제도 같이 사라진다
  (「화면 중앙 판정은 정확히 일치할 수 없다」) — `isCentered`와 2pt 허용 오차를 함께 지웠다
- 잃는 것은 「현재 화면의 중앙으로 되돌리기」다. 화면이 하나면 다음 화면이 자기 자신이라 그 동작이 그대로 남는다

## 설정 창의 초기 포커스는 `didBecomeKey` 다음에 잡힌다 (2026-08-28)

- AppKit은 창이 처음 key가 될 때 key view loop의 첫 컨트롤을 잡는다.
  설정 창은 아무것도 선택되지 않은 채로 열려야 해서 그때 한 번 풀어준다
- **그 선택은 `didBecomeKeyNotification`이 온 다음에 이뤄진다.** 알림 안에서 `makeFirstResponder(nil)`을 불러 봐야
  아직 잡히기 전이라 풀 것이 없다 — 한 박자 미뤄야 실제로 풀린다
...
- `Slider(in:)`의 범위는 노브 위치 계산에도 쓰인다. 상대 값에 따라 좁히면
  값이 그대로인 슬라이더의 노브가 조작 중에 미끄러진다 — 트랙은 절대 범위로 고정하고 벽은 `Binding`에서 세운다
- `Slider(step:)`은 스냅과 함께 스냅 지점마다 눈금 점을 그린다 — 점 없이 스냅만 남기려면 값을 반올림하는 `Binding`을 끼운다
- `Binding<Double>` 확장 안에서 `min` · `max`는 dynamic member lookup이 가로챈다 — `Swift.min`처럼 모듈을 붙인다

## 그래프 축 라벨 자리는 최악의 경우로 고정한다 (2026-08-28)

- 라벨 폭을 값에 따라 재면 조작 중에 플롯 폭이 흔들린다.
  가장 긴 속도 라벨(`6000`, caption2에서 26.5pt)에 간격 6pt를 더한 값으로 왼쪽 여백을 고정한다
- 축 양 끝의 라벨은 가운데 정렬하면 절반이 캔버스 밖으로 나간다 —
  첫 라벨은 `.topLeading`, 마지막은 `.topTrailing`으로 축 안쪽에 붙인다
- 양 끝의 눈금선은 그래프 테두리와 겹쳐 그리지 않는다
