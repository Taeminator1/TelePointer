...
완료하지 못한 App Store 제출 준비 · 검증 항목은 [v3/Tasks.md](../v3/Tasks.md)로 이관했다.
...
- [x] 기본 단축키 조합 결정 — ⌃⌥ + IJKL
    - [x] 7개 조합(방향 4 + 클릭 2 + 이동 1)이 서로, 그리고 다른 앱과 충돌하지 않는지
...
- [x] modifier를 먼저 떼거나 앱 전환이 끼어들어도 커서가 멈추는지 확인
...
- [x] 그래프 조작감 확인 — 제어점 잡기, 슬라이더를 움직일 때 축이 변하는 느낌
