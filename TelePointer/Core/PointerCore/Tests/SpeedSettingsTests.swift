...
struct SpeedSettingsTests {
    private let defaults = UserDefaults(suiteName: "SpeedSettingsTests")!

    init() {
        defaults.removePersistentDomain(forName: "SpeedSettingsTests")
    }

    @Test("저장된 값이 없으면 기본 곡선")
    func fallsBackToDefault() {
        #expect(SpeedSettings(defaults: defaults).curve == .default)
    }
    ...
    func persistsAcrossInstances() {
        ...
        SpeedSettings(defaults: defaults).curve = curve

        #expect(SpeedSettings(defaults: defaults).curve == curve)
    }

    @Test("허용 범위를 벗어난 값은 저장 전에 당겨온다")
    func normalizesOnWrite() {
        let settings = SpeedSettings(defaults: defaults)
        ...
        #expect(SpeedSettings(defaults: defaults).curve == settings.curve)
    }
