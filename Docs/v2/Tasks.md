...
- [ ] 이동 거리 결정 — 구현하면서 맞춘다
- [ ] 화면 경계 처리 결정 — 가장자리에서 멈출지, 인접 모니터로 넘어갈지
- [ ] 기본 단축키 조합 결정 ([Notes.md](../Notes.md) 후보표 참고)
    - [ ] 4개 조합이 서로, 그리고 ⌃⌥⌘C와 충돌하지 않는지
- [ ] `KeyboardShortcuts.Name` 4개 정의 (상 · 하 · 좌 · 우)
- [ ] 현재 좌표 + 방향 → 목표 좌표를 계산하는 순수 함수 + 단위 테스트
- [ ] 핫키 핸들러 등록
- [ ] 연타로 눌렀을 때 이동이 누락되지 않는지 확인
- [ ] 메뉴에 방향 이동 항목을 넣을지 결정 (넣으면 메뉴가 4개 늘어난다)
